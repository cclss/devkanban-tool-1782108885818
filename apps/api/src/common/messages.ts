...
export const MESSAGES = {
  ...
  send: {
    ...
    // 예약 발송 시각이 이미 지났음 — 탓하지 않고 다음 행동(미래 시각 재지정)을 안내.
    scheduledInPast: '예약 시각이 이미 지났어요. 지금 이후 시각으로 다시 정해 주세요.',
    // 예약 발송 시각 형식 오류(파싱 불가한 값) — 내부 형식 사정 노출 없이 재확인 안내.
    scheduledInvalid: '예약 시각을 확인해 주세요. 올바른 날짜·시간으로 다시 정해 주세요.',
    // 예약이 아닌 계약을 변경/취소하려 함 — 탓하지 않고 가능한 대상(예약 상태)을 안내.
    notScheduled: '예약된 계약이 아니에요. 예약 발송을 설정한 계약만 변경하거나 취소할 수 있어요.',
    // 예약 잡 정보를 더 이상 찾을 수 없어 재예약 불가(예: 비영속 폴백 재시작으로 유실).
    // 내부 사정 대신 회복 경로(취소 후 재설정)를 부드럽게 안내한다.
    scheduleUnavailable: '예약 정보를 불러올 수 없어요. 예약을 취소한 뒤 다시 설정해 주세요.',
  },
