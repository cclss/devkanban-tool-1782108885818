...
export const MESSAGES = {
  ...
  // 문서 자동 분석(AI 필드 배치) 결과 안내 — 200 응답의 meta.reason으로 내려간다.
  // 실패/빈 결과를 오류로 취급하지 않고, 다음 행동(직접 배치)을 부드럽게 안내한다.
  analyze: {
    // 분석은 정상 수행됐지만 배치할 만한 필드를 찾지 못한 경우.
    empty: '자동으로 배치할 필드를 찾지 못했어요. 필요한 위치에 필드를 직접 추가해 주세요.',
    // 문서를 읽거나 분석하지 못한 경우(손상·미지원 구조 등) — 내부 사정은 노출하지 않는다.
    failed: '문서를 분석하지 못했어요. 필요한 필드를 직접 배치해 주세요.',
  },
  send: {
