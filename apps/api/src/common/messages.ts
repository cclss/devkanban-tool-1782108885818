...
export const MESSAGES = {
  ...
  // 발신자 브랜딩 설정 — 관리자가 로고/컬러/폰트를 설정할 때 보는 카피.
  // 발신자 톤과 동일하게: 탓하지 않고, 다음 행동을 부드럽게 안내한다.
  branding: {
    // Team 미만 플랜의 접근 차단(403) — 제약을 알리고 다음 행동(업그레이드)을 안내.
    // 어떤 플랜인지/내부 권한 규칙은 노출하지 않는다.
    forbidden:
      '브랜딩 설정은 팀 플랜부터 사용할 수 있어요. 플랜을 업그레이드하면 바로 쓸 수 있어요.',
    // 컬러 값이 헥스 형식(#RRGGBB / #RGB)이 아님 — 형식만 부드럽게 안내.
    invalidColor: '브랜드 색상은 #RRGGBB 형식의 색상 코드로 입력해 주세요.',
    // 등록되지 않은 폰트 키 — 목록에서 다시 고르도록 안내(어느 값이었는지 비노출).
    invalidFont: '선택할 수 있는 폰트가 아니에요. 목록에서 다시 골라 주세요.',
    // 로고 파일이 비어 있음.
    logoEmpty: '이미지 파일이 비어 있어요. 다른 파일로 다시 시도해 주세요.',
    // 허용 형식(JPG/PNG/SVG) 외 또는 위조된 형식 — 허용 형식만 간결히 알린다.
    // (확장자·MIME 위조는 매직바이트로 걸러지며, 같은 카피로 거절한다.)
    logoInvalidType: 'JPG, PNG, SVG 이미지만 올릴 수 있어요.',
    // 2MB 초과 — 한도를 명확히 알리고 다음 행동(더 작은 파일)을 안내.
    logoTooLarge: '이미지가 너무 커요. 2MB 이하 파일로 올려 주세요.',
    // 헤더는 이미지지만 내용이 손상돼 처리할 수 없음 — 재시도 안내(내부 원인 비노출).
    logoUnreadable:
      '이미지를 읽을 수 없어요. 파일이 손상되지 않았는지 확인하고 다시 올려 주세요.',
    // 저장된 로고를 찾을 수 없을 때(서빙 경로).
    logoNotFound: '로고 이미지를 찾을 수 없어요.',
  },
  send: {
