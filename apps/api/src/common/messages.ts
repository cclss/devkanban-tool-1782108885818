...
export const MESSAGES = {
  ...
  // AI 조항 카드 카피 — 서명자 대면. 요약은 원문의 참고·리마인드용 보조 수단이며
  // 법적 효력은 원문에 있다(기획 제약). 아래 문구는 모두 고정 카피(단일 출처)로,
  // 서비스/프롬프트가 즉흥 생성하지 않는다. 톤 규칙·주의 taxonomy·"참고용" 위상
  // 결정은 design-spec conventions/messaging.md(M6~M8)에 기록.
  clause: {
    // "AI 요약=참고용" 보조 수단 고지. 카드 스택에 함께 노출한다.
    advisoryNotice:
      'AI가 핵심만 간추린 요약이에요. 참고용이며, 정확한 내용과 법적 효력은 계약 원문에 있어요.',
    // 주의 플래그 라벨 — 카드의 `cautionReason`으로 노출. LLM/스텁이 분류한
    // 카테고리를 이 고정 문구로 매핑한다(문구 즉흥 생성 금지). 탓하지 않고
    // 다음 행동(원문 확인)을 부드럽게 안내한다(M2 voice 정합).
    caution: {
      autoRenewal: '자동 갱신 조항이에요. 갱신·해지 시점을 미리 확인해 주세요.',
      earlyTerminationPenalty:
        '중도 해지 시 위약금이 있을 수 있어요. 조건을 확인해 주세요.',
      paymentObligation:
        '비용을 부담하는 조항이에요. 금액과 납부 시점을 확인해 주세요.',
      liability: '책임·배상에 관한 조항이에요. 내용을 꼼꼼히 확인해 주세요.',
      personalData: '개인정보 처리에 관한 조항이에요. 제공 범위를 확인해 주세요.',
      other: '한 번 더 살펴보면 좋은 조항이에요. 원문 내용을 확인해 주세요.',
    },
    // 휴리스틱 스텁 폴백(LLM 미설정)이 내는 결정적 카드의 제목·요약. LLM 요약과
    // 달리 스텁은 문장을 쓸 수 없으므로 카테고리별 고정 카피를 사용한다. 모든
    // 요약이 "정확한 내용은 원문에서 확인" 위상을 반복해 원문 정본성을 강화한다.
    stub: {
      autoRenewal: {
        title: '자동 갱신',
        summary:
          '별도 통지가 없으면 계약이 자동으로 갱신될 수 있어요. 갱신 시점과 해지 방법은 원문에서 확인해 주세요.',
      },
      earlyTermination: {
        title: '중도 해지',
        summary:
          '계약 기간 중 해지할 때의 조건을 담은 조항이에요. 위약금 여부와 통지 기간은 원문에서 확인해 주세요.',
      },
      payment: {
        title: '대금·비용',
        summary:
          '지급할 금액과 시점에 관한 조항이에요. 정확한 금액과 납부 방법은 원문에서 확인해 주세요.',
      },
      liability: {
        title: '책임·배상',
        summary:
          '손해나 배상 책임을 다루는 조항이에요. 책임 범위는 원문에서 확인해 주세요.',
      },
      personalData: {
        title: '개인정보',
        summary:
          '개인정보를 수집·제공하는 내용을 담은 조항이에요. 제공 범위와 목적은 원문에서 확인해 주세요.',
      },
      term: {
        title: '계약 기간',
        summary:
          '계약이 유지되는 기간에 관한 조항이에요. 시작일과 종료일은 원문에서 확인해 주세요.',
      },
      governingLaw: {
        title: '준거법·분쟁',
        summary:
          '분쟁이 생겼을 때 적용되는 법과 절차에 관한 조항이에요. 자세한 내용은 원문에서 확인해 주세요.',
      },
    },
  },
} as const;
