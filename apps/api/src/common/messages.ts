...
export const MESSAGES = {
  ...
  // 비밀번호 재설정 대면 메시지 — 로그인 못 하는 사용자가 비밀번호를 새로 정할 때 보는 카피.
  // findId와 동일한 보안 원칙: 계정 존재 여부를 드러내지 않는다(열거 방지). 요청 단계는
  // 매칭 여부와 무관하게 항상 동일한 generic 안내를, 검증 실패는 원인을 특정하지 않는 문구를
  // 쓴다. 재설정 토큰은 짧게 살고 한 번 쓰면 무효화되므로, 만료/사용 후 접근은 어느 쪽인지
  // 특정하지 않는 단일 안내로 처음부터 다시 받도록 유도한다. 톤은 findId/auth 카피와 동일.
  resetPassword: {
    // 요청 접수 — 매칭/미매칭 모두 동일하게 노출(계정 존재 여부 비노출).
    requestAccepted:
      '입력하신 정보로 가입된 계정이 있다면, 인증 코드를 보내 드렸어요. 잠시 후 메일함이나 문자를 확인해 주세요.',
    ...
    // 코드 불일치 — 어느 자리가 틀렸는지, 계정이 있는지 특정하지 않는다.
    codeMismatch: '인증 코드가 일치하지 않아요. 다시 확인해 주세요.',
    // 코드 만료(발급 후 짧은 TTL 경과) — 처음부터 다시 받도록 안내.
    codeExpired: '인증 코드가 만료됐어요. 처음부터 다시 시도해 주세요.',
    // 연속 실패로 인한 일시 잠금 — 시간 경과로 자동 해제됨을 알린다.
    locked: '인증을 여러 번 실패했어요. 잠시 후 다시 시도해 주세요.',
    // 본인 확인 완료 — 다음 행동(새 비밀번호 설정)으로 부드럽게 안내한다.
    verified: '본인 확인이 끝났어요. 새 비밀번호를 설정해 주세요.',
    // 새 비밀번호 설정 완료 헤드라인 — spec의 "…완료되었습니다!" 형식.
    completed: '비밀번호 변경이 완료되었습니다!',
    // 재설정 토큰 만료/무효 안내 — 짧은 TTL 경과 또는 1회 사용 후 무효화. 어느 쪽인지
    // 특정하지 않고(내부 사정 비노출) 처음부터 다시 받도록 유도한다.
    tokenInvalid:
      '비밀번호 재설정 정보가 만료됐거나 더 이상 유효하지 않아요. 처음부터 다시 시도해 주세요.',
    // 인증 코드 안내 이메일 카피(reset-password.template.ts가 렌더).
    codeEmail: {
      subject: '[전자계약] 비밀번호 재설정 인증 코드예요',
      headline: '비밀번호 재설정 인증 코드예요',
      intro: '아래 6자리 코드를 입력하면 본인 확인이 끝나요.',
      expiry: '이 코드는 5분 동안만 사용할 수 있어요.',
      disclaimer: '본인이 요청하지 않았다면 이 메일은 무시해 주세요.',
    },
    // 변경 완료 안내 이메일 카피(confirm 성공 후, 보안 알림). 강조할 값이 없어 highlight 없음.
    doneEmail: {
      subject: '[전자계약] 비밀번호가 변경됐어요',
      headline: '비밀번호 변경이 완료되었어요',
      intro: '요청하신 대로 비밀번호를 새로 설정했어요.',
      outro: '로그인 화면에서 새 비밀번호로 다시 로그인해 주세요.',
      disclaimer: '본인이 변경하지 않았다면 비밀번호를 다시 바꾸고 고객센터로 알려 주세요.',
    },
    // SMS/알림톡 본문 — 한 줄 텍스트. 톤 유지, 코드는 대괄호로 강조.
    sms: {
      code: (code: string) =>
        `[전자계약] 비밀번호 재설정 인증 코드는 [${code}]예요. 5분 안에 입력해 주세요.`,
      done: '[전자계약] 비밀번호가 변경됐어요. 본인이 변경하지 않았다면 고객센터로 알려 주세요.',
    },
  },
 ...
 * 비밀번호 재설정 정책.
 *
 * 보안 요구사항(짧은 TTL · 단일 사용 · 요청 쓰로틀)을 상수로 표현한다.
 * - 인증 코드 TTL은 5분으로 짧게 둬 탈취·재사용 위험을 줄인다(findId와 동일).
 * - 검증 실패 잠금은 서명자 본인확인과 동일한 정책(5회/15분)을 재사용한다
 *   (`SIGNER_VERIFY_MAX_ATTEMPTS`, `SIGNER_VERIFY_LOCK_WINDOW_MINUTES`).
 * - 요청(코드 발송)은 같은 타깃 기준 윈도우당 횟수를 제한해 스팸·메일 폭탄을
 *   막는다(findId 요청 쓰로틀과 동일 패턴; 초과 시에도 응답은 동일한 generic 안내).
 * - 본인확인 통과 후 발급하는 재설정 토큰은 코드보다도 짧게(10분) 살리고, 한 번
 *   사용하면(=새 비밀번호 확정) 즉시 무효화한다(단일 사용; 소비 처리는 grain-3
 *   서비스/영속성 계층의 책임).
 */
export const RESET_PASSWORD_CODE_TTL_MINUTES = 5;
/** 요청 쓰로틀 — 이 시간(분) 내 같은 타깃의 코드 발송 횟수를 센다. */
export const RESET_PASSWORD_REQUEST_WINDOW_MINUTES = 15;
/** 요청 쓰로틀 — 윈도우 내 허용하는 최대 코드 발송 횟수. */
export const RESET_PASSWORD_REQUEST_MAX_PER_WINDOW = 5;
/** 재설정 토큰 유효 시간(분) — 코드보다 짧게. 1회 사용 시 즉시 무효화(단일 사용). */
export const RESET_PASSWORD_TOKEN_TTL_MINUTES = 10;
