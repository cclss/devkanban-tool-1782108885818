...
export const MESSAGES = {
  ...
  document: {
    ...
    // 멀티파트 업로드는 PDF와 DOCX(서버에서 PDF로 변환)를 받는다 — 두 형식을 함께 안내.
    invalidFileType: 'PDF 또는 DOCX 파일만 업로드할 수 있어요.',
    // 형식과 무관하게 "빈 파일" 결과만 알리므로 특정 형식을 지목하지 않는다.
    emptyFile: '파일이 비어 있어요. 다른 파일로 다시 시도해 주세요.',
    corruptPdf: 'PDF를 읽을 수 없어요. 파일이 손상되지 않았는지 확인해 주세요.',
    // 크기 제한은 형식과 무관하므로(PDF·DOCX 공통) 형식 문구 없이 한도만 알린다.
    fileTooLarge: '파일이 너무 커요. 20MB 이하로 올려 주세요.',
    // 업로드한 DOCX를 서버에서 PDF로 변환하지 못했을 때(손상·비정상 파일 등).
    // 시나리오 원문("…변환하지 못했습니다…")은 -습니다체지만, 프로젝트 base voice는
    // 해요체 + 다음 행동이라 그 톤으로 채택한다. 내부 사정(soffice 실패 원인)은 감춘다.
    conversionFailed:
