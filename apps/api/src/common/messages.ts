...
export const MESSAGES = {
  ...
  // 회사 브랜딩 설정(브랜드 색상·폰트) 카피 — 발신자(관리자) 대면.
  ...
  branding: {
    ...
    // 로고 파일이 비어 있음 — 다시 선택하도록 안내(400).
    logoEmpty: '이미지 파일이 비어 있어요. 다른 파일로 다시 시도해 주세요.',
    // 허용하지 않는 로고 형식 — JPG/PNG/SVG만 가능. 올바른 형식을 알려 즉시 교정 유도(400).
    logoFormat: 'JPG, PNG, SVG 형식의 이미지만 올릴 수 있어요.',
    // 로고 파일 크기 초과 — 상한과 다음 행동(더 작은 파일)을 명확히 안내(400).
    logoTooLarge: '로고 이미지가 너무 커요. 2MB 이하의 이미지로 올려 주세요.',
  },
  send: {
