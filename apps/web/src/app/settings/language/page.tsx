...
export default function LanguageSettingsPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setSaveError(translateWeb(selected, 'settings.saveFailed'));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-lg">
      ...
      {saveError ? <div className="flex items-center justify-between gap-sm rounded-md bg-danger-subtle px-md py-sm text-sm text-danger" role="alert"><span>{saveError}</span><Button size="sm" variant="secondary" disabled={saving} onClick={() => void save()}>{t('settings.retry')}</Button></div> : null}
      {savedNotice ? <p className="rounded-md bg-success-subtle px-md py-sm text-sm text-success" role="status">{savedNotice}</p> : null}
