/**
 * Sign-field geometry — the **single source of truth** for the sign-field
 * coordinate contract shared across the monorepo.
 *
 * This package owns, in one place:
 *   • the coordinate-system convention — normalized `0..1`, **PDF bottom-left
 *     origin** (+x right, +y UP) for the stored/persisted shape,
 *   • the per-type default field footprints ({@link FIELD_TYPE_META}), and
 *   • the in-page normalization clamp ({@link clampNormRect}).
 *
 * Both sides of the contract consume this module so the convention can never
 * drift between them:
 *   • the web placement canvas (`apps/web`) re-exports it and adds nothing —
 *     drag/drop, zoom-invariant restore, snapping, resize;
 *   • the API (`apps/api`) reuses the normalized types, `FIELD_TYPE_META`, and
 *     `clampNormRect` for PDF field-suggestion heuristics.
 *
 ...
 *     0..1 relative to the page. This is the stored shape (`SignFieldDto`
 *     normalized 0..1) and what `pdf-lib` later consumes.
 *
